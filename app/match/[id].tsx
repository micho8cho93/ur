...
import { EmojiReactionMenu } from '@/components/game/EmojiReactionMenu';
import {
...
import {
  ...
  FORFEIT_RESULT_TRANSITION_DELAY_MS,
  MATCH_EXIT_TRANSITION_POST_DELAY_MS,
  MATCH_EXIT_TRANSITION_PRE_DELAY_MS,
  TOURNAMENT_AUTO_RETURN_COUNTDOWN_MS,
  TOURNAMENT_WAITING_ROOM_TRANSITION_DELAY_MS,
} from '@/src/match/transitionPresentation';
import {
...
import { resolveMatchScoreRankTitles } from '@/src/match/scoreRankTitles';
import { isTournamentBotUserId } from '@/shared/tournamentBots';
import type { CosmeticDefinition } from '@/shared/cosmetics';
import {
  ALL_EMOJI_REACTION_OPTIONS,
  DEFAULT_EMOJI_REACTION_OPTIONS,
  EQUIPPABLE_EMOJI_REACTION_OPTIONS,
} from '@/shared/emojiReactions';
...
const EMOJI_BY_KEY: Record<EmojiReactionKey, string> = ALL_EMOJI_REACTION_OPTIONS.reduce(
  (entries, option) => ({
    ...entries,
...
export function GameRoom() {
  ...
  const { user } = useAuth();
  const { equippedEmoteCosmetics } = useInventory();
  ...
  const playerRankTitle = user?.provider === 'guest' ? 'Laborer' : progression?.currentRank ?? null;
  ...
  const cosmeticPlayerColor = isOfflineLocalPvPMatch ? null : resolvedPlayerColor;
  ...
  const resolvedDidPlayerWin = resolveDidPlayerWin({
    winnerColor: gameState.winner,
    resolvedPlayerColor,
    authoritativeMatchEnd,
    userId: authenticatedUserId,
  });
  const didPlayerWin = resolvedDidPlayerWin === true;
  ...
  const opponentFeedbackLabel = useMemo(() => {
    if (!authoritativePlayers || !opponentUserId) {
      return null;
    }

    const opponentColor =
      authoritativePlayers.light.userId === opponentUserId
        ? 'light'
        : authoritativePlayers.dark.userId === opponentUserId
          ? 'dark'
          : null;
    ...
    const opponent = authoritativePlayers[opponentColor];
    return opponent.title ?? opponent.rankTitle ?? opponent.userId ?? opponentUserId;
  }, [authoritativePlayers, opponentUserId]);
  ...
  const [postResultCountdownMs, setPostResultCountdownMs] = React.useState<number | null>(null);
  ...
  const [showFeedbackComposer, setShowFeedbackComposer] = React.useState(false);
  ...
  const previousGameState = previousStateRef.current.state;
  const isBotRollVisualPending =
    isOfflineBotMatch &&
    previousStateRef.current.matchId === (matchId ?? null) &&
    previousGameState.currentTurn === 'dark' &&
    previousGameState.rollValue === null &&
    gameState.currentTurn === 'dark' &&
    gameState.rollValue !== null;
  const isRollingVisualActive = rollingVisual || isBotRollVisualPending;
  const equippedEmojiReactionOptions = useMemo(() => {
    const equippedEmotes = equippedEmoteCosmetics
      .filter((cosmetic): cosmetic is CosmeticDefinition => Boolean(cosmetic))
      .slice(0, EQUIPPABLE_EMOJI_REACTION_OPTIONS.length);

    return equippedEmotes.map((cosmetic, index) => ({
      key: EQUIPPABLE_EMOJI_REACTION_OPTIONS[index].key,
      emoji: EQUIPPABLE_EMOJI_REACTION_OPTIONS[index].emoji,
      label: cosmetic.name,
    }));
  }, [equippedEmoteCosmetics]);
  const emojiReactionOptions = useMemo(
    () => [...DEFAULT_EMOJI_REACTION_OPTIONS, ...equippedEmojiReactionOptions],
    [equippedEmojiReactionOptions],
  );
  ...
  const shouldPromptTournamentWaitingRoomAfterForfeit =
  ...
  const postResultAutoAction = useMemo<'enter_waiting_room' | 'return_home' | null>(() => {
    ...
    if (shouldAutoExitTournamentResultModal || shouldAutoExitOnlineForfeitModal) {
      return 'return_home';
    }

    return null;
  }, [
    shouldAutoExitOnlineForfeitModal,
    shouldAutoExitTournamentResultModal,
  ...
  const tournamentResultModalMessage = useMemo(() => {
    ...
    } else if (shouldPromptTournamentWaitingRoomAfterForfeit) {
      baseMessage = `${winModalMessage} Head to the waiting room when you are ready for the next round.`;
  ...
  const resultCountdownLabel = useMemo(() => {
    if (!postResultAutoAction || postResultCountdownMs === null) {
      return null;
    }

    const remainingSeconds = Math.max(0, Math.ceil(postResultCountdownMs / 1000));
    return postResultAutoAction === 'enter_waiting_room'
  ...
  }, [postResultAutoAction, postResultCountdownMs]);
  ...
  const reconnectGraceBannerText = useMemo(() => {
    if (!hasReconnectGraceActive) {
      return null;
    }

    return buildReconnectGraceBannerText({
      isOpponentReconnecting,
  ...
  }, [isRollingVisualActive, showHeldRollResult]);
  ...
  const triggerLocalRoll = React.useCallback((options?: { autoTriggered?: boolean }) => {
    if (showRulesIntroModal || !introsComplete || !canRoll || isRollingVisualActive || rollButtonLatchPhase !== 'idle') {
      return;
    }
  ...
  useEffect(() => {
    ...
    if (tutorialPendingActionStep.kind === 'ROLL') {
      if (gameState.phase !== 'rolling' || gameState.currentTurn !== 'dark' || isRollingVisualActive) {
        return;
      }
  ...
  useEffect(() => {
    const hasReachedResultState = isOffline
      ? gameState.winner !== null
      : authoritativeMatchEnd !== null || gameState.winner !== null || gameState.phase === 'ended';

    if (!hasReachedResultState || storedMatchId !== matchId) {
      return;
    }
  ...
  const exitMatchToHome = React.useCallback(
    async (
      options?: {
        refreshTournamentStatus?: boolean;
      ...
      if (options?.withTransition === false) {
        await finalizeExitMatchToHome({
          refreshTournamentStatus: options.refreshTournamentStatus,
        });
        return;
      }

      const completeExit = () =>
        finalizeExitMatchToHome({
          refreshTournamentStatus: options?.refreshTournamentStatus,
        });

      const didStart = await runScreenTransition({
        title: options?.title ?? 'Returning Home',
        message: options?.message ?? 'Closing the board and returning to the archive.',
        variant: options?.variant ?? (didPlayerWin ? 'success' : 'neutral'),
        preActionDelayMs: MATCH_EXIT_TRANSITION_PRE_DELAY_MS,
        postActionDelayMs: MATCH_EXIT_TRANSITION_POST_DELAY_MS,
        action: completeExit,
      });
  ...
  const handleEnterTournamentWaitingRoom = React.useCallback(async () => {
    ...
    enteringTournamentWaitingRoomRef.current = true;

    try {
      const didStart = await runScreenTransition({
        title: 'Advancing To Waiting Room',
        message: 'Settling the bracket before the next round begins.',
        variant: 'success',
        preActionDelayMs: TOURNAMENT_WAITING_ROOM_TRANSITION_DELAY_MS,
  ...
  const handleTournamentResultExit = React.useCallback(async (options?: { source?: 'manual' | 'auto' }) => {
    ...
    if (options?.source === 'auto') {
      ...
      try {
        await exitMatchToHome({
          ...
          message: 'Final standings are locked. Heading back to the archive.',
        });
        didExit = true;
      } finally {
        if (!didExit) {
    ...
    if (tournamentExitValidationFailed || !tournamentRunIdParam) {
      await exitMatchToHome({
        refreshTournamentStatus: true,
        title: 'Returning Home',
        message: 'Closing the finished tournament board and returning to the archive.',
      });
      return;
    }
  ...
  const handleExit = React.useCallback(() => {
    ...
    void exitMatchToHome({
      withTransition: showWinModal,
      title: 'Returning Home',
      message: showWinModal
        ? didPlayerWin
          ? 'Sealing the match result and heading back to the archive.'
          : 'Closing the finished board and heading back to the archive.'
        : 'Leaving the current board and returning to the archive.',
      variant: showWinModal && didPlayerWin ? 'success' : 'neutral',
    });
  }, [
    didPlayerWin,
    exitMatchToHome,
    handleEnterTournamentWaitingRoom,
    handleTournamentResultExit,
    isTournamentMatch,
    isTournamentResultModal,
    shouldPromptTournamentWaitingRoomAfterForfeit,
    showWinModal,
  ...
  useEffect(() => {
    if (!postResultAutoAction) {
      setPostResultCountdownMs(null);
      return;
    }

    const startedAt = Date.now();
    setPostResultCountdownMs(TOURNAMENT_AUTO_RETURN_COUNTDOWN_MS);

    const interval = setInterval(() => {
      const remainingMs = Math.max(0, TOURNAMENT_AUTO_RETURN_COUNTDOWN_MS - (Date.now() - startedAt));
      setPostResultCountdownMs(remainingMs);

      if (remainingMs <= 0) {
        clearInterval(interval);

        if (postResultAutoAction === 'enter_waiting_room') {
          void handleEnterTournamentWaitingRoom();
          return;
        }
  ...
  useEffect(() => {
    ...
    clearRollTimer();
    setRollingVisual(false);
  }, [clearRollTimer, gameState.phase, gameState.rollValue, rollButtonLatchPhase, isRollingVisualActive]);
  useEffect(() => {
    if (
      !diceAnimationEnabled ||
      !isOfflineBotMatch ||
    ...
    if (
      previousSnapshot.matchId !== (matchId ?? null) ||
      previousSnapshot.state.phase !== 'rolling' ||
      previousSnapshot.state.currentTurn !== 'dark' ||
      previousSnapshot.state.rollValue !== null
    ) {
      return;
    }
  ...
  useEffect(() => {
    ...
    if (previousJoinedPlayerCount >= 2 && joinedPlayerCount < 2) {
      replaceMatchCue('opponentDisconnected');
    }
  ...
  useEffect(() => {
    ...
    if (rollValueChanged && gameState.rollValue !== null && !shouldSkipResolvedRollAudio && !isRollingVisualActive && !isBotRoll) {
      void gameAudio.play('roll');
    }
  ...
  const useTraylessMobileWebReserveRails = isMobileWebLayout && useMobileSideReserveRails;
  ...
  const mobileReserveColumnWidth = useMobileSideReserveRails
    ? useTraylessMobileWebReserveRails
      ? Math.max(36, Math.min(48, Math.round(stageContentWidth * 0.1)))
      : Math.max(
    ...
    : 0;
  const mobileWebBoardTrayAlignmentBaseLift =
    isMobileWebLayout && useMobileSideReserveRails && !useTraylessMobileWebReserveRails
  ...
  const layoutBoardScale = useMobileSideReserveRails
  ...
  const mobileWebTopChromeDrop = isMobileWebLayout ? Math.max(10, Math.round(topChromeHeight * 0.18)) : 0;
  const topChromeTop = isWebLayout && boardTargetFrame
    ? Math.round(
      webTopChromeTopInset +
      mobileWebTopChromeDrop +
      Math.max(0, boardTargetFrame.y - webTopChromeTopInset - topChromeHeight) / 2,
    )
    : insets.top + mobileTopChromeOffset + mobileWebTopChromeDrop;
  ...
  const mobileReserveRailTopOffset = useTraylessMobileWebReserveRails
    ? 0
    : resolveMobileReserveRailTopOffset({
      isMobileWebLayout,
  ...
  const mobileReserveRailAlignedTopOffset = useTraylessMobileWebReserveRails
    ? 0
    : mobileReserveRailTopOffset + mobileWebBoardTrayAlignmentLift;
  const mobileWebUnderBoardControlReserve =
    isMobileWebLayout && useMobileSideReserveRails
      ? mobileWebRollButtonArtSize + urTheme.spacing.sm
      : 0;
  const mobileBoardViewportHeightBudget = isMobileLayout
    ? Math.max(
      220,
      viewportHeight -
  ...
  const viewportBoardWidthLimitByHeight = Math.min(
    urTheme.layout.boardMax,
    boardOuterPadding +
      (Math.max(0, mobileBoardViewportHeightBudget - boardOuterPadding - verticalBoardGapTotal) * verticalBoardCols) /
        verticalBoardRows,
  );
  const viewportFitBoardScale = Math.max(
    0.24,
    Math.min(
      mobileBoardScaleCap,
  ...
  useEffect(() => {
    if (!isMobileWebLayout || !useMobileSideReserveRails || useTraylessMobileWebReserveRails) {
  ...
  useEffect(() => {
    if (Platform.OS !== 'web' || typeof document === 'undefined' || !isMobileWebLayout) {
      return;
    }
  ...
  const pauseAmbientEffects = isRollingVisualActive || hasActiveBoardMoveAnimation;
  const showDestinationHighlights = introsComplete && !isRollingVisualActive && gameState.rollValue !== null;
  ...
  const showWebRollResult = introsComplete && showWebSideDiceVisual && !isRollingVisualActive && displayedRollValue !== null;
    ...
    >
      ...
      {showMobileWebDetachedDarkScore && mobileWebDetachedDarkScoreFrame ? (
        ...
        >
          <EdgeScore
            ...
            rankTitle={scoreRankTitles.dark}
          ...
          {showTopMenu && (
            <View style={styles.topMenu}>
              ...
              <Pressable
                ...
                style={({ pressed }) => [styles.topMenuItem, pressed && styles.topMenuItemPressed]}
              >
                <MaterialIcons name="flag" size={18} color={TOP_CHROME_ACCENT} />
                <Text style={styles.topMenuLabel}>Send Feedback</Text>
              </Pressable>
              <Pressable
                accessibilityRole="button"
      ...
      >
        <View style={[styles.stageWrap, { gap: stageGap }]}>
          ...
          {reconnectGraceBannerText ? (
            <View pointerEvents="none" testID="reconnect-grace-banner" style={styles.reconnectGraceBanner}>
          ...
          >
            <View pointerEvents="none" style={styles.scoreIndicatorSlot}>
              <EdgeScore
                ...
                rankTitle={scoreRankTitles.light}
          ...
          {useSideColumns ? (
            ...
            >
              ...
              >
                {useMobileSideReserveRails ? (
                  ...
                  >
                    <PieceRail
                      ...
                      tokenVariant="light"
                      orientation="vertical"
                      showTrayArt={!useTraylessMobileWebReserveRails}
                      piecePixelSize={scaledReservePiecePixelSize}
                      trayScale={reserveTrayScale}
                      reserveCount={lightReserve}
                      totalCount={pieceCountPerSide}
                ...
                >
                  <View
                    style={[styles.boardViewport, { maxHeight: mobileBoardViewportHeightBudget }]}
                    onLayout={(event) => {
                      const { width: slotWidth, height: slotHeight } = event.nativeEvent.layout;
                      setBoardSlotSize((prev) =>
                        prev.width === slotWidth && prev.height === slotHeight
      ...
      <FeedbackComposerModal
        ...
        sourcePage="match"
        initialType={opponentUserId ? 'player_report' : null}
        matchContext={matchId ? { matchId } : null}
        reportedUser={
          opponentUserId
            ? {
                userId: opponentUserId,
                username: opponentFeedbackLabel ?? opponentUserId,
              }
            : null
        }
...
const styles = StyleSheet.create({
  ...
  reconnectGraceBanner: {
    ...
    borderRadius: urTheme.radii.md,
    borderWidth: 1,
    borderColor: 'rgba(232, 98, 46, 0.34)',
    backgroundColor: 'rgba(47, 20, 15, 0.88)',
  },
  reconnectGraceBannerText: {
    color: 'rgba(247, 229, 203, 0.96)',
    fontSize: 13,
    lineHeight: 19,
    textAlign: 'center',
  },
